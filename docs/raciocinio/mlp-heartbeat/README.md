...
| X | [10-aceleracao-gpu-fedora-secure-boot.md](10-aceleracao-gpu-fedora-secure-boot.md) | Aceleração por GPU no Fedora 42 com Secure Boot. Diagnóstico completo do driver NVIDIA (akmod + MOK), instalação das bibliotecas CUDA via `tensorflow[and-cuda]`, correção do `LD_LIBRARY_PATH` no `~/.bashrc` para install `--user`. Speedup de 12.8x em benchmark matmul. |
| XI | [11-logs-tensorflow-correcao-vs-supressao.md](11-logs-tensorflow-correcao-vs-supressao.md) | Logs do TensorFlow: taxonomia das três mensagens originais, correção legítima da que sinalizava estado real, aceitação documentada das que são ruído de bootstrap e informação sobre otimizações. Princípio: corrigir causa, não ocultar efeito. |
| XII | [12-nao-determinismo-gpu.md](12-nao-determinismo-gpu.md) | Não-determinismo do TensorFlow em GPU: métricas variam entre execuções apesar das sementes fixadas, porque reduções paralelas do cuDNN e não-associatividade de ponto flutuante propagam divergências microscópicas até as métricas finais. Trade-off entre determinismo estrito (20-40% mais lento) e variação controlada aceita. Inclui protocolo de atualização de valores em documentação. |
