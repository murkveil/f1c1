...
| X | [10-aceleracao-gpu-fedora-secure-boot.md](10-aceleracao-gpu-fedora-secure-boot.md) | Aceleração por GPU na Fedora 42 com Secure Boot. Diagnóstico completo do driver NVIDIA (akmod + MOK), instalação das bibliotecas CUDA via `tensorflow[and-cuda]`, correção do `LD_LIBRARY_PATH` no `~/.bashrc` para install `--user`. Speedup de 12.8x em benchmark matmul. |
